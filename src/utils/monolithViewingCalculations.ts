 ...
 * Best when card is tilted to show monolith face-on
 */
export const calculateViewingAngle = (transform: Transform3D): number => {
  const { x: rotX, y: rotY } = transform.rotation;
  
  // Optimal range: slight tilt on X (10-30°), Y rotation for perspective (15-45°)
  const optimalRotX = 20; // degrees
  const optimalRotY = 30; // degrees
  
  const xScore = Math.max(0, 1 - Math.abs(rotX - optimalRotX) / 30);
  const yScore = Math.max(0, 1 - Math.abs(Math.abs(rotY) - optimalRotY) / 30);
