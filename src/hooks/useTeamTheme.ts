...
export const useTeamTheme = () => {
  ...
  const setCustomHeaderBgColor = useCallback((color: string, colorType: string) => {
    ...
    // Helper function to convert hex to HSL string (matches the one in applyTheme)
    const hexToHsl = (hex: string): string => {
      const r = parseInt(hex.slice(1, 3), 16) / 255;
      const g = parseInt(hex.slice(3, 5), 16) / 255;
      const b = parseInt(hex.slice(5, 7), 16) / 255;
    ...
    // Apply HSL conversion to any CSS variables that might be affected
    const root = document.documentElement;
    if (color.startsWith('#')) {
      const hslColor = hexToHsl(color);
      // Update CSS variables that might be used by navbar
