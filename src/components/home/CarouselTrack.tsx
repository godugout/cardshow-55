...
const EnhancedCardItem = memo(({ card, scrollVelocity, position, isVisible }: EnhancedCardItemProps) => {
  ...
  return (
    ...
    >
      <div 
        className="w-48 h-64 bg-crd-darker rounded-lg border border-crd-darkGray/20 overflow-hidden transition-all duration-500 ease-out"
        style={{
      ...
      >
        {/* Card Image */}
        <div className="w-full h-48 bg-crd-dark flex items-center justify-center overflow-hidden relative">
          {!isLoaded && (
            <div 
        ...
        {/* Card Info */}
        <div className="p-3 h-16 flex flex-col justify-center relative">
          <h3 
            className="text-sm font-medium text-crd-white truncate transition-all duration-300"
            style={{
...
export const CarouselTrack = memo(({ cards }: CarouselTrackProps) => {
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    ...
    const cardWidth = 200; // Approximate card width including margin
