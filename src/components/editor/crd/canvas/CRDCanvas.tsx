...
export const CRDCanvas: React.FC<CRDCanvasProps> = ({
  ...
  return <div className="relative h-full w-full overflow-hidden bg-transparent flex flex-col">
      ...
      <div className={`flex-1 w-full flex items-center justify-center relative z-10 pt-16 overflow-hidden ${isPanning ? 'cursor-grab' : 'cursor-default'} ${isDragging ? 'cursor-grabbing' : ''}`} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp}>
        <div className="relative transition-transform duration-200 ease-out" style={{
        transform: `translate(${panOffset.x}px, ${panOffset.y}px) scale(${zoom / 100})`
          ...
          return <>
                {/* Horizontal ruler */}
                <div className="absolute -top-8 left-0 w-full h-8 border-b-2 text-xs z-40" style={{
              ...
              color: rulerColors.text
                  ...
                  <div className="relative w-full h-full">
                    {/* Measurement ticks */}
                    {Array.from({
                  length: Math.ceil(cardWidth / 20)
                }, (_, i) => <div key={i} className="absolute bottom-0 border-l" style={{
                  left: `${i * 20}px`,
                  height: i % 5 === 0 ? '16px' : '8px',
                  borderLeftColor: rulerColors.tick
                }}>
                        {i % 5 === 0 && <span className="absolute -top-4 -left-2 text-xs font-mono font-semibold" style={{
                    color: rulerColors.text
                  }}>
                ...
                {/* Vertical ruler */}
                <div className="absolute -left-8 top-0 w-8 h-full border-r-2 text-xs z-40" style={{
              backgroundColor: rulerColors.background,
              ...
              color: rulerColors.text
                  ...
                  <div className="relative w-full h-full">
                    {/* Measurement ticks */}
                    {Array.from({
                  length: Math.ceil(cardHeight / 20)
                }, (_, i) => <div key={i} className="absolute right-0 border-t" style={{
                  top: `${i * 20}px`,
                  width: i % 5 === 0 ? '16px' : '8px',
                  borderTopColor: rulerColors.tick
                }}>
                        {i % 5 === 0 && <span className="absolute -right-6 -top-2 text-xs font-mono font-semibold transform -rotate-90 origin-center" style={{
                    color: rulerColors.text,
                ...
                <div className="absolute -top-8 -left-8 w-8 h-8 border-b-2 border-r-2 z-50 flex items-center justify-center" style={{
              backgroundColor: rulerColors.background,
              borderBottomColor: rulerColors.border,
              borderRightColor: rulerColors.border
            }}>
          ...
          <div className="relative rounded-xl shadow-2xl overflow-hidden transition-all duration-300 hover:shadow-3xl" style={{
          width: cardWidth,
          height: cardHeight,
