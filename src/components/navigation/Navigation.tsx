...
export const Navigation: React.FC<NavigationProps> = ({ className }) => {
  ...
  return (
    <nav className={cn("hidden lg:flex items-center gap-2 lg:gap-6", className)}>
      {navigationItems.map((item) => {
        ...
        return (
          <Link
            ...
            to={item.href}
            className={cn(
              "text-sm font-semibold uppercase tracking-normal px-2 lg:px-4 py-2 transition-colors duration-200 min-h-[44px] min-w-[44px] flex items-center justify-center",
              isActive 
                ? "text-crd-orange" 
